...
def _mad_thresholds(adata, cfg, log):
    ...
    if lo > med * 0.80:
        ...
        log.warning(
            ...
            '(tissue_maturity=%s).',
            lo_orig, med, med * 0.80, lo, cfg.tissue_maturity,
        )
        ...
        if len(nf) >= 10:
            ...
            if lo > med * 0.80:
                ...
                log.warning(
                    ...
                    'clamping to %.0f. Over-filtering risk in low-gene-count tissue '
                    '(tissue_maturity=%s).',
                    lo, med, med * 0.80, cfg.tissue_maturity,
                )
    return thresholds
...
def _suggest_qc_thresholds(adata, cfg, log):
    ...
    log.info("  [qc-suggest] nFeature distribution: median=%.0f, MAD=%.0f, "
             "P10=%.0f, P90=%.0f, peaks=%d%s",
             med, mad, p10, p90, n_peaks,
    ...
    if cfg.qc.max_genes < p90:
        ...
        log.warning(
            ...
            cfg.qc.max_genes, p90, pct_above, int(p90 * 1.2),
        )
    ...
    if abs(cfg.qc.min_genes - recommended_lo) > max(recommended_lo * 0.5, 100):
        log.info(
            ...
            recommended_lo, cfg.qc.min_genes, abs(cfg.qc.min_genes - recommended_lo),
        )
