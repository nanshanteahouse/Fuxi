...
def build_pseudobulk(
    adata, group_col: str, use_raw: bool = True, log: object = None
) -> pd.DataFrame:
...
def run_grn(pseudo_df: pd.DataFrame, net: pd.DataFrame, log: object) -> tuple:
    ...
    log.info(
        "Running ULM enrichment on %d cells x %d genes", pseudo_df.shape[0], pseudo_df.shape[1]
    )
