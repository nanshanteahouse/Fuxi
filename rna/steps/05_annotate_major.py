...
def _warn_if_low_coverage(adata, cfg, log):
    ...
    if n_unknown > 0 and n_unknown / adata.n_obs > 0.5:
        log.warning(
            "⚠️  %.0f%% of cells (%d/%d) annotated as 'Unknown' — KB may not cover this dataset",
            n_unknown / adata.n_obs * 100,
            n_unknown,
            adata.n_obs,
        )
    if pass_rate < cfg.marker.quality_gate_min_pass_rate:
        log.error(
            "QUALITY GATE FAILED: pass_rate=%.4f < threshold=%.4f — %d/%d cells are 'Unknown'. Aborting.",
            pass_rate,
            cfg.marker.quality_gate_min_pass_rate,
            n_unknown,
            adata.n_obs,
        )
...
def run_annotation(adata, marker_dict, logger):
    ...
    for ct in cell_types:
        ...
        if not genes_present:
            ...
            continue
        sc.tl.score_genes(
            adata, gene_list=genes_present, score_name=f"score_{ct}", random_state=42
        )
    ...
    if len(cell_types) >= 2:
        ...
        adata.obs["annotation_confidence"] = (
            adata.obs["leiden"].map(confidence).astype(float).values
        )
        low_conf = (adata.obs["annotation_confidence"] < 0.02).sum()
        if low_conf > 0:
            logger.info(
                "  Low-confidence cells (<0.02): %d (%.1f%%)",
                low_conf,
                100 * low_conf / adata.n_obs,
            )
...
def run_subclustering(adata, cfg, subcluster_types, resolution, min_cells, logger):
    ...
    for parent_type in subcluster_types:
        ...
        sub = adata[mask].copy()
        sc.pp.neighbors(sub, n_pcs=50, use_rep="X_pca_harmony", random_state=42)
        sc.tl.leiden(
            sub,
...
def ai_annotate(adata, cfg, logger, std=None):
    ...
    logger.info(
        "Adaptive max_tokens: n_clusters=%d → max_tokens=%d", n_clusters, cfg.ai.max_tokens
    )
    ...
    adata.obs["cell_subtype"] = leiden_str.map({k: v["subtype"] for k, v in annotations.items()})
    ...
    for rec in ann_records:
        logger.info(
            "  Cluster %s → %s (state=%s, subtype=%s, conf=%s)",
            rec["cluster"],
            rec["cell_type"],
            rec["state"],
            rec["subtype"],
...
def score_genes_mode(adata, cfg, logger):
    ...
    run_subclustering(
        ...
        cfg.marker.min_cells_subcluster,
        logger,
    )
...
def main():
    ...
    log = setup_logger("05_annotate_major", os.path.join(cfg.log_dir, "05_annotate_major.log"))
    ...
    if _dev_mode:
        ...
        cfg.marker.validation_marginal_threshold = getattr(
            cfg.marker, "validation_marginal_threshold", 0.25
        )
        if not hasattr(cfg, "_dev_overlap_set") or not cfg._dev_overlap_set:
            ...
            log.info(
                "  validation_marginal_threshold: 0.25 → %.2f",
                cfg.marker.validation_marginal_threshold,
            )
            log.info("  ai.unconstrained_annotation → True (allow novel/transitional types)")

    adata = sc.read(cfg.cluster_h5ad)
    log.info(
        "Loaded: %s — %d cells, %d clusters",
        cfg.cluster_h5ad,
        adata.n_obs,
