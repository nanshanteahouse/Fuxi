...
def main():
    ...
    parser.add_argument(
        "--config", default=None, help="Config file path (auto-reads h5ad_dir etc.)"
    )
    parser.add_argument(
        "--input",
        "--input-h5ad",
    ...
    strategy_group.add_argument(
        "--random", action="store_true", help="Equivalent to --strategy random"
    )
    ...
    target_group.add_argument(
        "--target-fraction", type=float, default=None, help="Retention fraction (0.0-1.0)"
    )
    target_group.add_argument(
        "--max-per-sample",
        type=int,
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Preview statistics only, do not downsample"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    if cfg is not None:
        if (
            args.target_total is None
            and args.target_fraction is None
            and args.max_per_sample is None
        ):
    ...
    if strategy != "max_per_sample":
        log.info(
            "Current: %d cells → target: %d cells (reduction %.1f%%)",
            n_cells,
            target_total,
    ...
    if args.dry_run:
        ...
        log.info(
            "Tip: if estimated memory is insufficient, suggest target_total ≤ %d",
            int(n_cells * 0.3),
        )
        return
    ...
    if cfg and args.to_config:
        log.info(
            "Tip: rename downsampled file to original checkpoint name for seamless continuation"
        )
