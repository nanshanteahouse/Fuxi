...
def run_pseudobulk_de(adata, cfg, log):
    ...
    if not pb.contrast_treatment or not pb.contrast_baseline:
        log.error(
            "de.pseudobulk.contrast_treatment and contrast_baseline "
            "must be non-empty",
        )
        return
    ...
    for val_name, val in [
        ...
        if val not in valid_levels:
            log.error(
                "de.pseudobulk.%s='%s' not found in adata.obs['%s']. "
                "Valid values: %s",
                val_name, val, pb.contrast_column, sorted(valid_levels),
            )
            return
    ...
    if adata.raw is not None:
        ...
        log.info(
            ...
            count_adata.n_obs, count_adata.n_vars,
        )
    else:
        count_adata = adata
        log.warning(
    ...
    for ct in cell_types:
        ...
        ct_condition_counts = (
            pdata.obs.loc[ct_mask, pb.contrast_column].value_counts()
        )

        if any(ct_condition_counts < pb.min_cells_per_group):
            log.warning(
                "Cell type '%s': insufficient samples per condition "
                "(%s), skipping",
                ct, dict(ct_condition_counts),
            )
            continue
        ...
        if ct_pdata.n_vars == 0:
            log.warning(
                "Cell type '%s': no genes remaining after filtering, "
                "skipping",
                ct,
            )
            continue
        ...
        try:
            ...
            if len(plot_df) == 0:
                log.warning(
                    "No valid padj values for cell type '%s', "
                    "skipping volcano",
                    ct,
                )
                plt.close()
            ...
            fig_path = os.path.join(
                fig_dir, f"pseudobulk_volcano_{ct}.png",
            )
    ...
    if all_results:
        ...
        log.info(
            ...
            combined_path, len(combined),
        )
    else:
        log.warning(
            "No pseudobulk DE results produced — check "
            "sample/contrast configuration",
        )
