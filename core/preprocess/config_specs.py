...
_VISIUM = FormatSpec(
    ...
    items=(
        ...
        SpecComment(
            (
                "data_format: seurat_csv  # 备选：Seurat CSV 多切片输入（GSE235583 风格：每样本目录 counts/md）",
                "samples: [AD3_D10]       # 配合 spatial.samples 按样本目录读入，deconvolution 默认 off",
            ),
            indent=0,
        ),
        _GLOBAL_NOTE,
        ...
        SpecComment(("",)),
        SpecComment(("Multi-slide (Seurat CSV: per-sample counts/md dirs)",), indent=1),
        SpecField("spatial.samples", value=[], comment="multi-slide sample dirs"),
        SpecField("spatial.symbol_mapping", value=False, comment="ENSG→symbol (mygene), optional"),
        SpecComment(("",)),
        ...
        SpecField("spatial.svg_method", value="moran", comment="moran (SPARK-X R-only, reserved)"),
        SpecComment(("run_segmentation: false",), indent=1),
        SpecComment(("",)),
        SpecComment(("Ambient decontamination (optional)",), indent=1),
        SpecField("spatial.decontamination", value="none", comment="none|cellbender|soupx"),
        SpecField(
            "spatial.decontamination_expected_cells",
            value=0,
            comment="CellBender --expected-cells (0=auto)",
        ),
        SpecComment(("",)),
        SpecComment(("Batch integration (spatial, optional)",), indent=1),
        SpecField(
            "spatial.integration_method",
            value="none",
            comment="none|harmony; overrides integration.method",
        ),
        SpecComment(("",)),
        SpecComment(("Spatial domains (optional)",), indent=1),
        SpecField(
            "spatial.domain_method", value="leiden_spatial", comment="none|leiden_spatial|stagate"
        ),
        SpecField("spatial.domain_resolution", value=1.0),
        SpecField("spatial.domain_n_neighbors", value=10, comment="STAGATE neighbors"),
        SpecField("spatial.stagate_rad_cutoff", value=150.0, comment="STAGATE radius"),
        SpecComment(("",)),
        SpecComment(("Deconvolution (cell2location, optional)",), indent=1),
        SpecField("spatial.deconv_method", value="cell2location", comment="none|cell2location"),
        SpecField("spatial.deconv_max_epochs", value=30000),
        SpecField("spatial.deconv_batch_size", value=256),
        SpecField("spatial.deconv_n_cells_per_location", value=8),
        SpecField("spatial.deconv_detection_alpha", value=20.0),
        SpecField("spatial.deconv_n_factors", value=0, comment=">0: NMF tissue zones"),
        SpecField(
            "spatial.deconv_ref_max_epochs", value=500, comment="reference RegressionModel epochs"
        ),
        SpecField(
            "spatial.deconv_ref_n_samples", value=1000, comment="reference posterior samples"
        ),
        SpecField("spatial.deconv_ref_batch_size", value=2500),
        SpecComment(("",)),
        SpecComment(("── Dataset metadata ──",)),
        SpecField("tissue", placeholder="TISSUE"),
        SpecComment(
