...
```
**Manual:** Copy from `templates/config_templates/` (generated by `python -m core.config scaffold`; do not edit by hand — edit `core/preprocess/config_specs.py` and regenerate) to `projects/{modality}/{GSE_ID}/config_<GSE_ID>.yaml`.

**Schema → config workflow.** `core/config/schema.py` is the single source of truth for config fields/defaults; starter templates are rendered from `core/preprocess/config_specs.py` specs. When adding a schema field: optionally add it to the relevant spec(s) in `config_specs.py`, then run `python -m core.config scaffold` and the test suite (stale templates fail `tests/test_core/test_config_scaffold.py`).
