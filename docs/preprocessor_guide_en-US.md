...
1. Use `python -m core.config scaffold --list` to see available formats; `--format <KEY> --out <path>` renders a starter config (from `core/preprocess/config_specs.py` specs — never hand-edit the specs, regenerate instead)
