...
def _mad_thresholds(adata, cfg, log):
    ...
    if lo > med * 0.80:
        ...
        log.warning(
            ...
            lo_orig, med, med * 0.80, cfg.tissue_maturity,
        )
    ...
    log.info("  n_genes_by_counts: median=%.0f, MAD=%.0f  ->  (lo=%.0f, hi=%.0f)  [adaptive x%.1f, %s]",
    ...
    if cfg.expression_type == "raw_counts":
        ...
        log.info("  total_counts:       median=%.0f, MAD=%.0f  ->  hi=%.0f  [adaptive x%.1f]",
    ...
    if cfg.expression_type == "raw_counts":
        ...
        if lo > cfg.qc.min_genes_per_umi:
            ...
            log.warning('  MAD lower bound (%.4f) exceeds min_genes_per_umi (%.4f) — clamping.',
                        lo_orig, cfg.qc.min_genes_per_umi)
        thresholds['log_genes_per_umi'] = (lo, None)
        log.info("  log_genes_per_umi:  median=%.4f, MAD=%.4f ->  lo=%.4f  [adaptive x%.1f, %s]",
...
def _plot_qc_diagnostics(adata, thresholds, output_dir, log):
    ...
    try:
        ...
        if ncount_hi is not None:
            ax.axvline(ncount_hi, color='orange', linestyle='--', linewidth=1.0, label=f'nCount hi={ncount_hi:.0f}')
...
def main():
    ...
    if CFG.qc.use_adaptive_thresholds:
        ...
        if adata.n_obs == 0:
            log.warning("ZERO survivors with MAD thresholds — reloading raw data with hard thresholds.")
            adata = sc.read(input_path)
