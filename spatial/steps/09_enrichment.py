...
def main():
    ...
    log.info(
        "Loaded marker genes: %d rows, %d groups", len(marker_df), marker_df["group"].nunique()
    )
    ...
    if os.path.exists(quality_path):
        ...
        if pass_rate < getattr(cfg.marker, "validation_pass_rate_min", 0.1):
            log.warning(
                ...
                pass_rate * 100,
                cfg.marker.validation_pass_rate_min * 100,
            )
    ...
    for gs in cfg.enrichment.gene_sets:
        ...
        if cfg.enrichment.method in ("ora", "both"):
            ...
            try:
                ...
                log.warning(
                    "ORA enrichment not available — rna/steps/09_enrichment.py may need to be runnable first"
                )
