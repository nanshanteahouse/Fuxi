    ...
    for grp in markers_df[group_col].unique() if group_col else ["all"]:
        ...
        if len(genes_clean) < 5:
            if len(genes_clean) == 0 and genes:
                log.warning(
                    "Enrichment: %s — no genes mapped (check pybedtools or gene_annotation_bed config)",
                    str(grp),
                )
            else:
                log.info(
                    "Enrichment: %s — too few genes (%d), skipping", str(grp), len(genes_clean)
                )
            continue
        genes = genes_clean
