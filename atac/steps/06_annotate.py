...
def main():
    ...
    log.info(
        ...
        data.n_obs,
        data.obs.get("leiden").nunique() if "leiden" in data.obs else 0,
    )
    ...
    data.obs["cell_type"] = (
        data.obs[cluster_col]
        .astype(str)
        .map(lambda x: annotations.get(x, {}).get("cell_type", f"Cluster_{x}"))
    )
    data.obs["annot_confidence"] = (
        data.obs[cluster_col]
        .astype(str)
        .map(lambda x: annotations.get(x, {}).get("confidence", "medium"))
    )
