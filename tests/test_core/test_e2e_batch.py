...
def scenario_1() -> None:
    ...
    assert "tissue" in report.biology_cols, (
        f"FAIL 1: 'tissue' not in biology_cols {report.biology_cols}"
    )
...
def scenario_2() -> None:
    ...
    adata = ad.AnnData(
        X=csr_matrix(x_raw),
        obs=pd.DataFrame(
            {"batch_x": pd.Categorical(labels), "disease": pd.Categorical(labels.copy())}
        ),
    )
...
def scenario_3() -> None:
    ...
    assert len(report.column_diagnoses) == 0, (
        f"FAIL 3: expected 0 diagnoses, got {len(report.column_diagnoses)}"
    )
...
def scenario_4() -> None:
    ...
    ad_wrong = adata.copy()
    ho_w = harmonypy.run_harmony(
        before.obsm["X_pca"], obs, "tissue", verbose=False, random_state=42
    )
    ...
    ho_c = harmonypy.run_harmony(
        before.obsm["X_pca"], obs, "sample", verbose=False, random_state=42
    )
...
def scenario_5() -> None:
    ...
    adata = ad.AnnData(
        ...
        obs=pd.DataFrame(
            {
                "batch": pd.Categorical(rng.choice(["A", "B"], 100)),
