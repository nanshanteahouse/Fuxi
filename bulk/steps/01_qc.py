...
def main():
    ...
    log.info("  Total counts:  min=%.0f  median=%.0f  max=%.0f",
             adata.obs["total_counts"].min(),
             adata.obs["total_counts"].median(),
             adata.obs["total_counts"].max())
    log.info("  Genes detected: min=%d  median=%d  max=%d",
    ...
    min_genes = getattr(CFG.bulk, "min_genes_per_sample", 5000) if not hasattr(CFG.bulk, "min_genes_per_sample") else 5000
    ...
    if n_flagged > 0:
        ...
        for name in adata.obs_names[flagged]:
            log.warning("  %s: %d counts, %d genes",
                        name,
    ...
    fig, ax = plt.subplots(figsize=(max(8, len(sample_names) * 0.8),
                                     max(6, len(sample_names) * 0.6)))
