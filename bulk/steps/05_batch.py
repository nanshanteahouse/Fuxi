...
def main():
    ...
    if batch_col not in adata.obs.columns:
        log.warning(
            ...
            batch_col, list(adata.obs.columns),
        )
        return
    ...
    sample_info = adata.obs[[batch_col, contrast_col]].copy() if contrast_available \
    ...
    _pca_comparison(adata.X, out_adata.X, adata.obs, batch_col,
                    CFG.figure_dir, log)
...
def _pca_comparison(X_before, X_after, obs, batch_col, figure_dir, log):
    ...
    try:
        ...
        if n_comps < 2:
            log.warning("Skipping PCA comparison: need ≥2 samples (got %d)",
                        X_before.shape[0])
            return
        ...
        for label in obs[batch_col].unique():
            ...
            ax1.scatter(pca_before[mask, 0], pca_before[mask, 1],
                        c=[colors[label]], label=label, edgecolors="black",
                        linewidths=0.5, s=60)
            ax2.scatter(pca_after[mask, 0], pca_after[mask, 1],
                        c=[colors[label]], label=label, edgecolors="black",
                        linewidths=0.5, s=60)
