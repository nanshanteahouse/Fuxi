...
def main():
    ...
    if lfc_col not in sig_df.columns:
        ...
        log.info("Up-regulated DEGs: %d  |  Down-regulated DEGs: %d",
    ...
    if not os.path.exists(all_de_path):
        ...
        if len(rank_series) < CFG.enrichment.min_size:
            log.info("Skip GSEA: %d ranked genes < min_size=%d",
                     len(rank_series), CFG.enrichment.min_size)
        else:
            try:
                gsea_res = gp.prerank(
                    rnk=rank_series,
                ...
                if gsea_res.res2d is not None and not gsea_res.res2d.empty:
                    gsea_res.res2d.to_csv(
                        os.path.join(CFG.table_dir, "03_gsea.csv"), index=False)
...
def _dot_ora(df: pd.DataFrame, direction: str, CFG, log):
    ...
    overlap_ratio = overlap_parts.str[0].astype(float) / overlap_parts.str[1].astype(float).clip(lower=1)
    ...
    ax.scatter(x, range(len(sig)),
               s=overlap_ratio * 200 + 20, c=overlap_ratio,
               cmap="Blues", edgecolors="grey", linewidths=0.5)
...
def _dot_gsea(df: pd.DataFrame, CFG, log):
    ...
    fig, ax = plt.subplots(figsize=(9, max(3.5, 0.3 * len(sig))))
    sc = ax.scatter(log10_fdr, range(len(sig)),
                    s=log10_fdr * 20, c=sig["NES"],
